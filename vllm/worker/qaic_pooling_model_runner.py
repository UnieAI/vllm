...
class QaicPoolingModelRunner(QaicModelRunner):
    ...
    def __init__(
        ...
        _qaic_cfg = vllm_config.additional_config if isinstance(vllm_config.additional_config, dict) else {}
        if _qaic_cfg:
            if "pooling_device" in _qaic_cfg:
                self.is_pooler = True
                if _qaic_cfg["pooling_device"] == 'qaic':
                    self.is_qaic_pooler = True
                    normalize = _qaic_cfg.get("normalize", False)
                    if (isinstance(normalize, str) and normalize.lower() in ('true', '1')) or normalize is True:
                        self.normalize = True
                    softmax = _qaic_cfg.get("softmax", False)

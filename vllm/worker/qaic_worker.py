...
class QaicWorker(LocalOrDistributedWorkerBase):
    ...
    def init_device(self) -> None:
        ...
        _dev_grp = (self.vllm_config.additional_config or {}).get("device_group") if hasattr(self, 'vllm_config') else None
        if _dev_grp is not None:
            for device_id in _dev_grp:
    ...
    def initialize_cache(self, num_gpu_blocks: int,
        ...
        _seq_len = int((self.vllm_config.additional_config or {}).get("max_seq_len_to_capture")
                       or (self.vllm_config.additional_config or {}).get("prefill_seq_len")
                       or self.model_config.max_model_len)
        self.cache_config.block_size = _seq_len
        self.num_phy_kv_cache_blks = num_gpu_blocks
        assert (num_gpu_blocks >= self.scheduler_config.max_num_seqs), (
                "num-gpu-blocks should be greater than equal to max-num-seqs")
        self.cache_config.num_gpu_blocks = self.num_phy_kv_cache_blks * cdiv(self.model_config.max_model_len,

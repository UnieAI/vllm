...
try:
    ...
    LoRAModulePath = None  # type: ignore[assignment, misc]
from peft import PeftConfig
try:
    from vllm.model_executor.layers.sampler import Sampler, SamplerOutput
except ImportError:
    from vllm.v1.sample.sampler import Sampler as _V1Sampler
    Sampler = _V1Sampler  # type: ignore[misc]
    SamplerOutput = object  # type: ignore[assignment, misc]
try:
    from vllm.model_executor.sampling_metadata import SamplingMetadata
except ImportError:
    from vllm.v1.sample.metadata import SamplingMetadata  # type: ignore[no-redef]
...
try:
    ...
    _CONFIG_REGISTRY = {}  # type: ignore[assignment]
try:
...
def get_hf_model(model_config: ModelConfig, qaic_config: Optional[dict] = None, lora_config=None, kv_offload=False):
    ...
    args = {
        "continuous_batching": qaic_config.get('continuous_batching', True),
    ...
    if model_config.runner_type == "pooling" and not model_config.is_multimodal_model:
        ...
        del args["kv_offload"]
        if qaic_config:
            if 'pooling_device' in qaic_config:
                if qaic_config['pooling_device'] == 'qaic':
                    args["pooling"] = qaic_config['pooling_method']
...
def _get_qaic_compile_config(
    ...
    if vllm_config.model_config.runner_type == "pooling" and not vllm_config.model_config.is_multimodal_model:
        _pool_seq_len = vllm_config.model_config.max_model_len
    else:
        _pool_seq_len = None

    _qaic_cfg = (vllm_config.additional_config
                 if isinstance(vllm_config.additional_config, dict) else {})
    _seq_len = int(_qaic_cfg.get("max_seq_len_to_capture")
                   or _qaic_cfg.get("prefill_seq_len")
                   or vllm_config.model_config.max_model_len)

    # Prepare default config
    cfg = {
        "qpc_path": None,
        "prefill_seq_len": _pool_seq_len if _pool_seq_len else _seq_len,
        "ctx_len": vllm_config.model_config.max_model_len,
        "batch_size": 1,
        "full_batch_size": vllm_config.scheduler_config.max_num_seqs,
        "kv_cache_batch_size": kv_cache_batch_size,
        "device_group": _qaic_cfg.get("device_group"),
        "num_devices": len(_qaic_cfg.get("device_group") or []) or 1,
        "num_cores": None,
    ...
    _additional_cfg = vllm_config.additional_config if isinstance(vllm_config.additional_config, dict) else {}
    cfg.update(_clean_config(_additional_cfg))
    ...
    if cfg["num_cores"] is None:
        ...
        if vllm_config.speculative_config and "draft" in vllm_config.speculative_config.method:
            ...
            if speculative_model_type =="target" and vllm_config.speculative_config.draft_override_qaic_config is not None:
                ...
                # update default settings
                _target_additional = getattr(vllm_config.speculative_config, 'target_model_config', None)
                _target_additional = getattr(_target_additional, 'override_qaic_config', {}) or {}
                other_cfg.update(_clean_config(_target_additional))
...
def update_hf_config(model_config: ModelConfig, qaic_cfg: dict = None):
    ...
    if hasattr(model_config.hf_config, "max_dynamic_patch"):
        if qaic_cfg and "num_patches" in qaic_cfg:
            use_thumbnail = model_config.hf_config.use_thumbnail
            num_patches = int(qaic_cfg["num_patches"])

...
class SpecDecodeBaseSampler(nn.Module):
    ...
    def _create_output(
        ...
        # compute cumulative product of accepted_tokens along each batch
        # so that we only count number of accepted tokens till the first
        # False value for each row.
        accepted_cumprod = torch.cumprod(accepted, dim=1)
        self.num_accepted_tokens += accepted_cumprod.sum()

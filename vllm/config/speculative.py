...
class SpeculativeConfig:
    ...
    # Jacobi decoding configuration
    jacobi_num_blocks: int = 1
    """The number of Jacobi blocks (K) to draft per step."""
    jacobi_steps_per_yield: int = 1
    """The number of Jacobi refinement steps to run before yielding."""
    jacobi_ngram_pool_size: int = 0
    """The size of the Jacobi ngram pool used for tail filling."""
    jacobi_prefill_random: bool = False
    """Whether to initialize Jacobi drafts with random tokens."""
    jacobi_max_batch_size: int | None = None
    """Maximum batch size for Jacobi decoding. If exceeded, Jacobi is disabled."""
    jacobi_waiting_running_ratio_high: float | None = 2.5
    """High threshold for waiting/running ratio to disable Jacobi."""
    jacobi_waiting_running_ratio_low: float | None = 1.8
    """Low threshold for waiting/running ratio to re-enable Jacobi."""
    jacobi_accept_rate_low: float | None = None
    """Low acceptance rate threshold for Jacobi DSC gating."""
    jacobi_accept_rate_high: float | None = None
    """High acceptance rate threshold for Jacobi DSC gating."""
    jacobi_accept_rate_ema_decay: float = 0.9
    """EMA decay for Jacobi acceptance rate tracking."""
    jacobi_accept_rate_warmup: int = 0
    """Warmup steps for Jacobi acceptance rate gating."""
    jacobi_accept_rate_probe_interval: int = 0
    """Probe interval when Jacobi DSC gating is closed."""
    ...
    def __post_init__(self):
        ...
        if self.method is None and (
            self.model is not None and self.model in ("ngram", "[ngram]", "jacobi")
        ):
    ...
    def _validate_jacobi(self):
        if self.jacobi_num_blocks < 1:
            raise ValueError(
                f"jacobi_num_blocks={self.jacobi_num_blocks} must be >= 1"
            )
        if self.jacobi_steps_per_yield < 1:
            raise ValueError(
                f"jacobi_steps_per_yield={self.jacobi_steps_per_yield} must be >= 1"
            )
        if self.jacobi_ngram_pool_size < 0:
            raise ValueError(
                f"jacobi_ngram_pool_size={self.jacobi_ngram_pool_size} must be >= 0"
            )
        if self.jacobi_max_batch_size is not None and self.jacobi_max_batch_size < 1:
            raise ValueError(
                f"jacobi_max_batch_size={self.jacobi_max_batch_size} must be >= 1"
            )
        if not 0 < self.jacobi_accept_rate_ema_decay <= 1:
            raise ValueError(
                f"jacobi_accept_rate_ema_decay={self.jacobi_accept_rate_ema_decay} "
                "must be in (0, 1]"
            )
        if self.jacobi_accept_rate_warmup < 0:
            raise ValueError(
                f"jacobi_accept_rate_warmup={self.jacobi_accept_rate_warmup} "
                "must be >= 0"
            )
        if self.jacobi_accept_rate_probe_interval < 0:
            raise ValueError(
                f"jacobi_accept_rate_probe_interval="
                f"{self.jacobi_accept_rate_probe_interval} must be >= 0"
            )
        if self.jacobi_accept_rate_low is not None and not (
            0 <= self.jacobi_accept_rate_low <= 1
        ):
            raise ValueError(
                f"jacobi_accept_rate_low={self.jacobi_accept_rate_low} must be in [0, 1]"
            )
        if self.jacobi_accept_rate_high is not None and not (
            0 <= self.jacobi_accept_rate_high <= 1
        ):
            raise ValueError(
                f"jacobi_accept_rate_high={self.jacobi_accept_rate_high} must be in [0, 1]"
            )
        if (
            self.jacobi_accept_rate_low is not None
            and self.jacobi_accept_rate_high is not None
            and self.jacobi_accept_rate_low > self.jacobi_accept_rate_high
        ):
            raise ValueError(
                "jacobi_accept_rate_low must be <= jacobi_accept_rate_high "
                f"({self.jacobi_accept_rate_low} > {self.jacobi_accept_rate_high})"
            )
        if (
            self.jacobi_waiting_running_ratio_high is not None
            and self.jacobi_waiting_running_ratio_high <= 0
        ):
            raise ValueError(
                "jacobi_waiting_running_ratio_high must be > 0 "
                f"({self.jacobi_waiting_running_ratio_high})"
            )
        if (
            self.jacobi_waiting_running_ratio_low is not None
            and self.jacobi_waiting_running_ratio_low <= 0
        ):
            raise ValueError(
                "jacobi_waiting_running_ratio_low must be > 0 "
                f"({self.jacobi_waiting_running_ratio_low})"
            )
        if (
            self.jacobi_waiting_running_ratio_low is not None
            and self.jacobi_waiting_running_ratio_high is not None
            and self.jacobi_waiting_running_ratio_low
            > self.jacobi_waiting_running_ratio_high
        ):
            raise ValueError(
                "jacobi_waiting_running_ratio_low must be <= "
                "jacobi_waiting_running_ratio_high "
                f"({self.jacobi_waiting_running_ratio_low} > "
                f"{self.jacobi_waiting_running_ratio_high})"
            )
        if (
            self.num_speculative_tokens is not None
            and self.num_speculative_tokens % self.jacobi_num_blocks != 0
        ):
            raise ValueError(
                "num_speculative_tokens must be divisible by "
                f"jacobi_num_blocks ({self.num_speculative_tokens} % "
                f"{self.jacobi_num_blocks} != 0)"
            )
    ...
    def __repr__(self) -> str:
        ...
        model = (
            None
            if method in ("ngram", "suffix", "jacobi")

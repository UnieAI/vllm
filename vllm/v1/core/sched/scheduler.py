...
class Scheduler(SchedulerInterface):
    ...
    def _jacobi_waiting_running_ratio_allows(self) -> bool:
        ...
        if not speculative_config or speculative_config.method != "jacobi":
            return True
        high = speculative_config.jacobi_waiting_running_ratio_high
        low = speculative_config.jacobi_waiting_running_ratio_low
        ...
        ratio = (len(self.waiting) + running) / running
        if self.jacobi_waiting_running_gate_open:
            if high is not None and ratio > high:
                logger.info(
                    "Disable Jacobi due to high waiting/running ratio: %.2f > %.2f",
                    ratio,
                    high,
                )
            ...
            if threshold is not None and ratio < threshold:
                logger.info(
                    "Enable Jacobi due to low waiting/running ratio: %.2f < %.2f",
                    ratio,
                    threshold,
                )
                self.jacobi_waiting_running_gate_open = True
    ...
    def update_draft_token_ids(self, draft_token_ids: DraftTokenIds) -> None:
        allow_jacobi = self._jacobi_waiting_running_ratio_allows()

...
class GPUModelRunner(
    ...
    def __init__(
        ...
        if self.speculative_config and get_pp_group().is_last_rank:
            ...
            elif self.speculative_config.method == "jacobi":
                self.drafter = JacobiProposer(self.vllm_config, self.device)
    ...
    def propose_draft_token_ids(
        ...
        elif spec_config.method == "jacobi":
            assert isinstance(self.drafter, JacobiProposer)
            assert logits is not None, "Jacobi requires logits for draft updates."
            draft_token_ids = self.drafter.propose(
                self.input_batch,
                self.requests,
                logits,

...
class QaicModelRunner(GPUModelRunner):
    def __init__(
        ...
        # QAIC uses host device handling, so device may be None
        assert device is None or device == torch.device("cpu"), \
            f"QaicModelRunner expects cpu or None device, got {device}"
        ...
        # max_seq_len_to_capture moved to additional_config in v0.21
        _qaic_cfg = (self.vllm_config.additional_config
                     if isinstance(self.vllm_config.additional_config, dict) else {})
        _seq_len = (_qaic_cfg.get("max_seq_len_to_capture")
                    or _qaic_cfg.get("prefill_seq_len")
                    or self.model_config.max_model_len)
        self.max_seq_len = int(_seq_len)
    ...
    def _postprocess_tensors(self) -> None:
        ...
        # v0.21: input_ids is CpuGpuBuffer (int32); create int64 CPU alias for QAIC
        self.input_ids_cpu = torch.zeros_like(self.input_ids.cpu, dtype=torch.int64)
        ...
        # v0.21: block_table.cpu is CpuGpuBuffer (int32); cast in-place
        bt = self.input_batch.block_table[0].block_table
        bt.cpu = bt.cpu.to(torch.int64)
        bt.np = bt.cpu.numpy()
        # v0.21: positions is a CPU torch.Tensor; expose as numpy alias for QAIC
    ...
    def _prepare_qaic_inputs(
        ...
        _arange_out = np.empty(total_num_scheduled_tokens, dtype=np.int32)
        cu_num_tokens = self._get_cumsum_and_arange(
            num_scheduled_tokens, _arange_out)
        arange = _arange_out[:total_num_scheduled_tokens]
    ...
    def execute_model(
        ...
        if not scheduler_output.total_num_scheduled_tokens:
            if not has_kv_transfer_group():
                ...
                self._last_model_runner_output = EMPTY_MODEL_RUNNER_OUTPUT
        ...
        self._last_model_runner_output = result
        return result
    ...
    def get_supported_tasks(self):
        # QAIC always supports text generation
        return ("generate",)

    def sample_tokens(self, grammar_output=None):
        """QAIC sampling is done inside execute_model; return cached result."""
        return getattr(self, '_last_model_runner_output', EMPTY_MODEL_RUNNER_OUTPUT)

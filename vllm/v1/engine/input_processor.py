...
class InputProcessor:
    ...
    def _validate_supported_sampling_params(
        ...
        if spec_config is not None and spec_config.method == "jacobi":
            self._validate_jacobi_params(params)

    def _validate_jacobi_params(self, params: SamplingParams) -> None:
        if params.logprobs is not None or params.prompt_logprobs is not None:
            raise ValueError("Jacobi decoding does not support logprobs.")
        if (
            params.structured_outputs is not None
            and not params.structured_outputs.all_constraints_none()
        ):
            raise ValueError("Jacobi decoding does not support structured outputs.")
        if params.allowed_token_ids:
            raise ValueError("Jacobi decoding does not support allowed_token_ids.")
        if params.bad_words:
            raise ValueError("Jacobi decoding does not support bad_words.")
        if params.temperature > _SAMPLING_EPS:
            raise ValueError("Jacobi decoding requires greedy sampling.")
        if params.top_p < 1.0:
            raise ValueError("Jacobi decoding requires top_p=1.0.")
        if params.top_k not in (0, 1, -1):
            raise ValueError("Jacobi decoding requires top_k<=1.")
        if params.min_p > _SAMPLING_EPS:
            raise ValueError("Jacobi decoding requires min_p=0.0.")
        if params.frequency_penalty != 0.0:
            raise ValueError("Jacobi decoding requires frequency_penalty=0.0.")
        if params.presence_penalty != 0.0:
            raise ValueError("Jacobi decoding requires presence_penalty=0.0.")
        if params.repetition_penalty != 1.0:
            raise ValueError("Jacobi decoding requires repetition_penalty=1.0.")
        if params.logit_bias:
            raise ValueError("Jacobi decoding does not support logit_bias.")
        if params.n != 1:
            raise ValueError("Jacobi decoding requires n=1.")

...
def ddtree_verify(
    ...
    Args:
        posterior_token_ids:   int[batch, budget+1] — sampled/verifier-chosen
            token ids for every tree node plus the next bonus token.
    ...
    Returns:
        DDTreeVerifyOutput with:
          - output_token_ids: int32[batch, budget+1] accepted path tokens +
            bonus token, -1 padded.
          - accepted_node_indices: accepted tree node indices per request,
            including the root node at index 0.
    """
    posterior_cpu = posterior_token_ids.cpu().tolist()
    ...
    return DDTreeVerifyOutput(
        output_token_ids=output.to(device),

...
from vllm.spec_decode.qaic_multi_step_worker import QaicMultiStepWorker
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    def create_worker(
        ...
        if ngram_prompt_lookup_max > 0:
            ...
            if draft_model_config.hf_config.model_type == "mlp_speculator":
            ...
            elif draft_model_config.hf_config.model_type == "turbo" and current_platform.is_qaic():
                proposer_worker = MedusaWorker(**draft_worker_kwargs)
            else:
                if draft_tp == 1:
                    if current_platform.is_cuda_alike():
                ...
                if current_platform.is_qaic():
                    proposer_worker = QaicMultiStepWorker(**draft_worker_kwargs)
                else:
        ...
        if current_platform.is_qaic():
            ...
            # we cache speculation logits at `proposer_worker.model_runner`
            if scorer_worker.speculative_model_type == "turbo":
                proposer_worker.model_runner = scorer_worker.model_runner
        elif not disable_mqa_scorer:
    ...
    def _run_speculative_decoding_step(
        ...
        # TODO: add num_acc_tokens idx to `seq_group_metadata_list` and read it on next decode proposal iteration

...
class Top1Proposer(SpeculativeProposer):
    ...
    def get_spec_proposals(
        ...
        if current_platform.is_qaic():
            # We assume all sequences have speculative decoding enabled:
            proposal_lens = [proposal_len] * len(seq_group_metadata_list)
            nonzero_proposal_len_indices = list(range(len(seq_group_metadata_list)))
            nonzero_proposal_len_seqs = seq_group_metadata_list
        else:
    ...
    def _merge_outputs(
        ...
        """
        fillvalue_proposal_token = -1
        fillvalue_proposal_len = 0
        if current_platform.is_qaic():
            fillvalue_proposal_len = proposal_len
            fillvalue_proposal_token = 0
        if maybe_sampler_output is None:
            # If no speculative tokens, the sampler output will be None.
            # In this case we return empty proposals.
            proposal_tokens = torch.tensor(fillvalue_proposal_token,
                                           dtype=torch.long,
            ...
            proposal_probs = torch.tensor(0,
                                          ...
                                          device=self._device).expand(
                                              batch_size, proposal_len,
                                              self._vocab_size)
            proposal_lens_tensor = torch.tensor(fillvalue_proposal_len,
                                                dtype=torch.long,
                                                device=self._device).expand(
                                                    len(proposal_lens))
        ...
        entire_proposal_tokens = proposal_tokens.new_full(
            ...
            fill_value=fillvalue_proposal_token,
        )
        ...
        proposal_lens_tensor = fillvalue_proposal_len * torch.ones(batch_size,
                                           dtype=torch.long,
                                           device=self._device)

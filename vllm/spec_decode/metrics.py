...
class AsyncMetricsCollector:
    ...
    def __init__(self,
        ...
        if not current_platform.is_qaic():
            ...
            self._copy_stream: Optional[torch.cpu.Stream] = None

        if not current_platform.is_qaic():
            self._in_flight_copy: Optional[torch.cuda.Event] = None
        else:
            self._in_flight_copy: Optional[torch.cpu.Event] = None
    ...
    def init_gpu_tensors(self, rank: int) -> None:
        ...
        if not current_platform.is_qaic():
            ...
            self._copy_stream = torch.cpu.Stream()
    ...
    def _copy_rejsample_metrics_async(self) -> Union[torch.cuda.Event, torch.cpu.Event]:
        ...
        if not current_platform.is_qaic():
            ...
            self._copy_stream.wait_stream(torch.cpu.current_stream())
        ...
        if not current_platform.is_qaic():
            ...
            with torch.cpu.stream(self._copy_stream):
                self._aggregate_num_accepted_tokens.copy_(
                    self.spec_decode_sampler.num_accepted_tokens,
                    non_blocking=True)
                self._aggregate_num_emitted_tokens.copy_(
            ...
            aggregate_metrics_ready = torch.cpu.Event()
    ...
    def _collect_rejsample_metrics(
            self, k: int,
            ready_event: Union[torch.cuda.Event, torch.cpu.Event]) -> SpecDecodeWorkerMetrics:

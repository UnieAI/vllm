...
class QaicMultiStepWorker(QaicWorker, ProposerWorkerBase):
    ...
    def __init__(self, *args, **kwargs):
        kwargs.update(speculative_model_type="draft")
        super().__init__(*args, **kwargs)
        # Lazy initialization list.
        self._proposer: SpeculativeProposer
        self.num_speculative_tokens = kwargs.get("vllm_config").speculative_config.num_speculative_tokens

    def init_device(self) -> None:
        super().init_device()

        #agokhale workaround for speculations stopping just before exhausting the context length: set max_proposal_len to num_speculative_tokens more than the max_model_len
        self._proposer = Top1Proposer(
            weakref.proxy(self),  # type: ignore[arg-type]
            None,
            self.vocab_size,
            max_proposal_len=self.max_model_len + self.num_speculative_tokens,
        )
    ...
    def sampler_output(
        ...
        #FIXME for qaic we dont need to expand the batch, we can directly run sequence length 2
        if len(seq_ids_with_bonus_token_in_last_step) > 0:
            for sgm in execute_model_req.seq_group_metadata_list:
                #indicate to the model runner that we want to run precode inference with last two output tokens
                # the other sequences in the batch will be padded with -1 if they did not have bonus tokens
                sgm.is_precode = True
        ...
        #skip _filter_model_outputs() since we are already indicating is_precode=True
        # for sequences with bonus ids in last iteration
    ...
    def _expand_execute_model_request(
        ...
        for seq_group in execute_model_req.seq_group_metadata_list:
            ...
            if seq_group_has_bonus_tokens:
                ...
                updated_seq_group_without_bonus_token  = \
                    QaicMultiStepWorker._copy_seq_metadata_excluding_last_token(
                        seq_group, seq_with_bonus_token_in_last_step)
                updated_seq_group_metadata_list.append(
                    updated_seq_group_without_bonus_token)
            # Add the original sequence group.
            updated_seq_group_metadata_list.append(
                QaicMultiStepWorker._shallow_copy_seq_group_metadata(seq_group))
        ...
        assert updated_execute_model_req.previous_hidden_states == None

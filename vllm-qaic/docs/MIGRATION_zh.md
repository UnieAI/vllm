...
> **先讲清楚版本:**这里有两条版本轴,不要混在一起说。
> - **来源轴:** QAIC patch 来自 v0.10.1 fork lineage(`bd90d0d` + `909a809`)。
> - **目标轴:** 本目录在做 vLLM 0.21/0.22 的树外插件迁移。
> - **当前本地基线:** `git describe` 是 `v0.21.1rc0-162-g...`;它不是 exact
>   `v0.21.0` tag,而且 `v0.21.0` 不是当前分支祖先。
>
> 所以目前只能说:**v0.10.1 QAIC patch 的 0.21+ 迁移已盘点,runtime 主路径已搬进
> `vllm-qaic` 并完成本地语法检查;尚未证明可在 exact vLLM 0.21.0 tag 或当前 0.21+
> 基线上完整跑通。**
...
**当前 repo 的诚实状态(截至本文件对应的 `roy/qaic-plugin`):**

| 项目 | 状态 | 证据/说明 |
|---|---|---|
| 改动盘点与迁移设计 | ✅ 已完成 | 本文件 + `MIGRATION_GPUModelRunner_old_vs_new.md` |
| 插件外壳 | 🟡 已写未上机验证 | `pyproject.toml` entry points、`vllm_qaic/__init__.py`、`platform.py` |
| ngram helper 代码 | 🟡 已写未上机验证 | 7 个 CPU rejection sampler helper + `_pack_decode_batch` 已在 `model_runner.py` |
| `get_kv_cache_spec` / `initialize_kv_cache` / `load_model` 外层方法 | 🟡 已写未上机验证 | 方法在 `model_runner.py`;`load_model` 已接到 ported `load_qaic_model` |
| `session.py` / `model_loader.py` / `compile_config.py` / `qserve_model_runner.py` / `worker.py` | 🟡 已搬入未上机验证 | 已从 `v1_ngram` 搬进插件并改 import;本地 `py_compile` 通过 |
| `execute_model` 主流程 | 🟡 已写未上机验证 | 已用 0.21 `_prepare_inputs` + `InputBatch` 重建 QAIC decode/prefill path;需 AIC 上机验证 |
| torch 2.7/2.11 分离方案 | 🔬 待验证 | Gate 1/2 脚本已备,必须在 AIC host 上跑 |
| `vllm serve` 功能跑通 | ⬜ 尚未完成 | 代码路径已补;仍依赖 AIC 上机验证 |
...
| `vllm/model_executor/model_loader/qserve_model_runner.py` | 🔵 搬运 | → `vllm_qaic/qserve_model_runner.py`。`qaic.py` / `qaic_v1.py` 的依赖,upstream 没有。 |
...
### 4.E 低风险方法(签名小改,外层已对齐)
`load_model`、`get_kv_cache_spec`、`initialize_kv_cache` 的**外层方法和 0.21 签名**
已放进 `vllm_qaic/model_runner.py`。当前状态:

- `load_model` 已接到从 fork 搬来的 `vllm_qaic/model_loader.py::load_qaic_model`。
- `session.py`、`compile_config.py`、`qserve_model_runner.py` 已搬入并改成本插件 import。
- `get_kv_cache_spec` / `initialize_kv_cache` / `load_model` 还需要在目标 vLLM + AIC host
  上实测。

### 4.G `qserve_model_runner.py` 能不能直接搬?

结论:**可以直接搬,但直接搬只解决 import/runtime dependency,不等于已经证明可上卡跑。**

原因:

- `qaic.py` / `qaic_v1.py` 会 import fork 里的 `qserve_model_runner.py`。
- upstream vLLM 没有这个文件,所以如果不搬,loader import 会直接失败。
- 这个文件主要是 QEfficient/qaicrt runner helper,不是深度耦合 vLLM 0.21 core 的文件;
  因此正确做法就是搬进 `vllm_qaic/qserve_model_runner.py`,并改 import:
  `qaic_session_np` → `vllm_qaic.session`,其他 package path → `vllm_qaic.*`。

它仍然依赖这些外部事实:

- AIC host 有可 import 的 `qaicrt` / `QAicApi_pb2`。
- QPC 是由匹配的 QEfficient + Cloud AI SDK 编译出来的。
- QPC input/output shape 与 loader 传入的 numpy buffer 对得上。

所以「直接搬」是**必要且合理**的代码移植方式;「可跑」仍要以 Gate 2 和 `vllm serve`
smoke test 证明。
...
| 涉及文件 | 内容 | 迁移状态 |
|---|---|---|
| `qaic_model_runner.py` | 7 个 CPU 拒绝采样函数 + 2D 打包 + ngram 开关 | 🟡 已接入 `vllm_qaic/model_runner.py::execute_model`;未上机验证 |
| `qaic_v1.py` | 让 target QPC 多输出 `N+1` 行打分 | 🟡 已搬进 `vllm_qaic/model_loader.py`;未上机验证 |
...
3. ✅ 低风险搬运已完成:`session.py`、`model_loader.py`、`compile_config.py`、
   `qserve_model_runner.py`、`worker.py` 已进入插件并通过语法检查。
4. 🟡 输入准备/`execute_model` 已按 0.21 `InputBatch` 改写;下一步在 AIC host
   **先把不带投机的 prefill/decode 跑通**。
5. 🟡 ngram 的 2D 打包 + CPU 拒绝采样已接进 `execute_model`;基础路径跑通后再开 ngram
   smoke test。
...
- 我们已经把 v0.10.1 QAIC patch 到 vLLM 0.21+ 的迁移范围完整盘点。
- 方案是树外插件,优先不改 vLLM core。
- ngram CPU rejection sampler 和 2D decode packing 这类自包含逻辑已搬到插件文件中。
- QAIC loader/session/worker/qserve dependency 已搬进插件并完成本地语法检查。
- 当前仍需 AIC host 做 Gate 1/2 和 runtime path 验证。

**现在还不能说:**

- 不能说「QAIC 0.10.x patch 已经可以跑在 exact vLLM 0.21.0」。
- 不能说「`vllm serve` 已跑通」。
- 不能说「AIC runtime 已验证」,因为目前只做了本地语法检查,还没在 AIC host
  跑 Gate 2 / serve smoke。
- 不能把当前分支称为 `v0.21.0` 基线;它是 `v0.21.1rc0` 之后的本地分支。

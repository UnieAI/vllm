...
> **當前 HEAD：** `7a6948ad3 Merge pull request #10 from UnieAI/roy/qaic-host-path-optimizations`  
> **分叉點：** `ca4ea2172`（來自 `origin/roy/tool-use-stream`）  
> **改動範圍：** 全部集中在 `vllm-qaic/` 目錄，主 vLLM core 未修改  
> **整體統計：** 22 個文件變更，新增 8,705 行
>
> **Branch 關係說明：**  
...
> 以及本地 vLLM 0.21 QAIC 相容修正（`cea6e0ceb`、`9d49743e6`）。
> 最新 HEAD 另合入 PR #10 的 host path optimization（`53a2f29c0`、`91b292000`）。
...
### 12.6 PR #10：QAIC host path optimization 與 profiling

2026-06-03 將 `origin/roy/qaic-host-path-optimizations` 合入 `v1_vllm021`：

```text
7a6948ad3  Merge pull request #10 from UnieAI/roy/qaic-host-path-optimizations
53a2f29c0  Optimize QAIC host-side batch preparation
91b292000  Add gated QAIC execution profiling
```

**實際改動範圍：**

```text
vllm-qaic/vllm_qaic/model_loader.py | 11 ++---
vllm-qaic/vllm_qaic/model_runner.py | 88 ++++++++++++++++++++++++++-----------
```

#### `model_loader.py`：重用 inactive batch index buffer

在 `QaicCausalLM.__init__()` 中新增兩個可重用的 host-side buffer：
...
`_run_decode()` 補 inactive decode rows 的 `batch_index` 時，原本每次都用 Python `set` 和 list comprehension 找未使用 slot：
...
這不改變 decode 語意：active rows 仍使用 scheduler 給的 `batch_indices`，inactive rows 仍填合法但本輪未使用的 slot id；差別是避免在每輪 decode 重建 set/list，降低 host-side batch preparation overhead。

#### `model_runner.py`：slot mapping 從 Python loop 改成 tensor vectorized

`_qaic_update_slot_mapping()` 原本逐 request、逐 token 用 Python loop 計算：

- token position 所在的 virtual block
- 該 request 的 block number
- context parallel interleave 下此 token 是否屬於本 rank
- local block offset
- 最終 `slot_mapping[offset]`
...
另外加了 `num_tokens == 0` 的 fast return，並保留 `slot_mapping[num_tokens:max_num_tokens].fill_(PAD_ID)`，確保未使用 token 位置仍被清成 padding。

這是本次 optimization 的核心：把原本容易卡在 Python interpreter 的 host-side slot mapping 準備，改為 torch tensor 批次運算。

#### `model_runner.py`：新增 `VLLM_QAIC_PROFILE`
...
當 `VLLM_QAIC_PROFILE=1`（或 `true` / `yes` / `on`）時，`execute_model()` 會額外記錄 QAIC 單步執行的分段耗時：

- `card`：QAIC model execution（decode + prefill）
- `logits`：`compute_logits`
- `sample`：sampling / speculative rejection sampling
- `bookkeeping`：`_bookkeeping_sync`

log 格式：

```text
QAIC-PROF reqs=<n> decodes=<n> prefills=<n> decode_tokens=<n> prefill_tokens=<n> logits_shape=<shape> | card=<ms> logits=<ms> sample=<ms> bookkeeping=<ms>
```
...
VLLM_QAIC_PROFILE=1 \
VLLM_USE_V1=1 \
VLLM_QAIC_QPC_PATH=/path/to/qpc \
.venv-vllm021/bin/python ...
```

#### 改動性質總結

本次 PR #10 不改 QAIC QPC 執行語意，主要是兩類補強：

| 類型 | 說明 |
|---|---|
| Host-side optimization | 減少 decode inactive slot 計算的 Python allocation，並將 slot mapping 計算 vectorize |
| Profiling | 透過 `VLLM_QAIC_PROFILE` 開關輸出 QAIC card / logits / sample / bookkeeping 分段耗時 |

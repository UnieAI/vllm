...
def _qaic_additional_config(vllm_config: VllmConfig) -> Dict[str, Any]:
    """Return QAIC knobs from vLLM 0.21's out-of-tree config channel.

    The v0.10.1 fork carried these on ModelConfig.override_qaic_config and
    DeviceConfig.device_group.  Upstream vLLM 0.21 has neither field, so the
    plugin accepts the same keys through VllmConfig.additional_config.
    """
    cfg = getattr(vllm_config, "additional_config", None) or {}
    if not isinstance(cfg, dict):
        return {}
    return dict(cfg)


def _model_qaic_config(model_config: ModelConfig) -> Dict[str, Any]:
    return dict(getattr(model_config, "override_qaic_config", None) or {})
...
class QaicCausalLM(nn.Module):
    ...
    def __init__(
        ...
        self.sampler = Sampler(logprobs_mode=vllm_config.model_config.logprobs_mode)
...
def get_hf_model(model_config: ModelConfig, qaic_config: Optional[dict] = None, lora_config=None, kv_offload=False):
    ...
    model_qaic_config = _model_qaic_config(model_config)
    args = {
        "continuous_batching": model_qaic_config.get('continuous_batching', True),
    ...
    if model_config.runner_type == "pooling" and not model_config.is_multimodal_model:
        ...
        del args["qaic_config"]
        del args["kv_offload"]
        if model_qaic_config:
            if 'pooling_device' in model_qaic_config:
                if model_qaic_config['pooling_device'] == 'qaic':
                    args["pooling"] = model_qaic_config['pooling_method']
...
def _get_qaic_compile_config(
    ...
    cfg = {
        ...
        "num_devices": len(device_group) if device_group is not None else 1,
        "num_cores": None,
    ...
    cfg.update(_clean_config(qaic_extra_config))
    vllm_config.additional_config = _clean_config(qaic_extra_config)
    ...
    if cfg["num_cores"] is None:
        ...
        if vllm_config.speculative_config and "draft" in vllm_config.speculative_config.method:
            ...
            target_override = (
                _model_qaic_config(target_model_config)
                if target_model_config is not None else {}
            )
            if speculative_model_type == "target" and draft_override is not None:
                    other_cfg.update(_clean_config(draft_override))
                    other_cfg.update(_clean_config({"device_group":os.environ.get("VLLM_QAIC_SPEC_DRAFT_QID", None)}))
            else:
                # update default settings
                other_cfg.update(_clean_config(target_override))
...
def update_hf_config(model_config: ModelConfig):
    ...
    if hasattr(model_config.hf_config, "max_dynamic_patch"):
        model_qaic_config = _model_qaic_config(model_config)
        if model_qaic_config and "num_patches" in model_qaic_config:
            use_thumbnail = model_config.hf_config.use_thumbnail
            num_patches = int(model_qaic_config["num_patches"])

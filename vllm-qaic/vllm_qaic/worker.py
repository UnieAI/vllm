...
from vllm_qaic.model_runner import QaicModelRunner
...
class QaicWorker(WorkerBase):
    ...
    def init_device(self) -> None:
        ...
        device_group = self._get_device_group()
    ...
    def _get_device_group(self) -> list[int] | None:
        qaic_config = self.vllm_config.additional_config or {}
        if not isinstance(qaic_config, dict):
            return None
        device_group = qaic_config.get("device_group")
        if device_group is None:
            return None
        if isinstance(device_group, int):
            return [device_group]
        if isinstance(device_group, str):
            return [int(x) for x in device_group.split(",") if x]
        return [int(x) for x in device_group]
    ...
    def sample_tokens(self, grammar_output: "GrammarOutput") -> ModelRunnerOutput:
        raise RuntimeError("QaicModelRunner returns ModelRunnerOutput directly.")

    def get_cache_block_size_bytes(self) -> int:
        page_size = get_uniform_page_size(self.get_kv_cache_spec())
        return page_size * self.model_config.get_num_layers(self.parallel_config)

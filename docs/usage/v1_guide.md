...
!!! note "Jacobi (lookahead) speculative decoding"

    Jacobi decoding is supported in vLLM v1 via `--speculative-config` with
    `{"method": "jacobi", "num_speculative_tokens": ...}`. It currently requires
    greedy sampling and does not support logprobs, structured outputs,
    `allowed_token_ids`, or `bad_words`. If any request violates these constraints,
    the step falls back to standard decode.

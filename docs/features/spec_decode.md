...
## Speculating with Jacobi (lookahead) decoding

Jacobi Decoding (lookahead decoding) proposes multiple tokens using the target
model itself, then verifies how many of those tokens can be accepted in a
single step. This is a greedy-only method that uses full logits and can
optionally recycle draft tails via an n-gram pool.

!!! warning
    Jacobi decoding in vLLM v1 currently requires greedy sampling only and does
    not support logprobs or structured output constraints. If any request in
    the batch violates these constraints, that step falls back to standard
    decoding.
...
??? code
    ...
    llm = LLM(
        ...
        speculative_config={
            ...
            "jacobi_num_blocks": 2,
            "jacobi_steps_per_yield": 1,
            "jacobi_ngram_pool_size": 8,
            "jacobi_prefill_random": False,
            "jacobi_max_batch_size": 4,
            "jacobi_accept_rate_low": 0.30,
            "jacobi_accept_rate_high": 0.40,
            "jacobi_accept_rate_ema_decay": 0.9,
            "jacobi_accept_rate_warmup": 100,
            "jacobi_accept_rate_probe_interval": 50,
            "jacobi_waiting_running_ratio_high": 2.5,
            "jacobi_waiting_running_ratio_low": 1.8,
        },
    )
    outputs = llm.generate(prompts, sampling_params)
...
### Jacobi configuration notes

- `num_speculative_tokens` sets the total lookahead length. It must be divisible
...
- Dynamic Strategy Control (DSC) gates can disable Jacobi at runtime:
  ...
  - `jacobi_waiting_running_ratio_*`: disable under queue pressure.
